<?php
    ...
    $query = $connection->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '$db' AND TABLE_NAME = '$table_name'");

    //$next_row = $result->fetch_assoc();
    //echo $next_row;

    while ($next_row = $connection->fetch_assoc())
    {
        $result[] = $next_row;
        echo $next_row;
    }
